...
const ReportComponent = () => {
  ...
  const img_path = './src/images/intel.jpg';
  const reportData = [
    { id: 1, name: 'John Doe', age: 28, job: 'Engineer', path:'', img: './src/images/apple.jpg'},
    { id: 2, name: 'ສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີສະບາຍດີ', age: 34, job: 'Designer', path:'', img: img_path},
    { id: 3, name: 'Sam Green', age: 45, job: 'Manager', path:'', img: img_path},
    { id: 4, name: 'John Doe', age: 28, job: 'Engineer', path:'', img: img_path},
    { id: 5, name: 'ສະບາຍດີ', age: 34, job: 'Designer', path:'', img: img_path},
    { id: 6, name: 'Sam Green', age: 45, job: 'Manager', path:'', img: img_path},
    { id: 7, name: 'John Doe', age: 28, job: 'Engineer', path:'', img: img_path},
    { id: 8, name: 'ສະບາຍດີ', age: 34, job: 'Designer', path:'', img: img_path},
    { id: 9, name: 'ສະບາຍດີ', age: 34, job: 'Designer', path:'', img: img_path},
    { id: 10, name: 'Sam Green', age: 45, job: 'Manager', path:'', img: img_path},
    { id: 11, name: 'John Doe', age: 28, job: 'Engineer', path:'', img: img_path},
    { id: 12, name: 'ສະບາຍດີ', age: 34, job: 'Designer', path:'', img: img_path}
  ];
  const headers = [
    { label: 'ID', key: 'id' },
    { label: 'Name', key: 'name' },
  ...
  const exportToPDF = async() => {
    ...
    // add font
    doc.addFont('./src/fonts/Phetsarath_OT.ttf', 'MyFont', 'normal');
    // set font header and logo
    doc.setFont("MyFont");
    doc.setFontSize(20);
    doc.setTextColor("black");
    doc.text('ລາຍງານພະນັກງານ[Employee]', 15, 15);
    doc.addImage("./src/images/intel.jpg", "JPEG", 170, 5, 15, 15);
    // Get the current date and time
    const now = new Date();
    const dateString = now.toLocaleString(); // Customize the format as needed
    // Add date and time to the PDF
    doc.setFontSize(12);
    doc.text(`Date/Time: ${dateString}`, 15, 25);
    // add line
    // doc.setLineWidth(0.5); // set linewidth size
    // doc.setLineDash([2.5]);// dash line
    // doc.line(15, 28, 195, 28);// set height and width
     
    // Column
    const tableColumn = ['ID', 'Name', 'Age', 'Job', 'Image'];
    // Rows
    const tableRows = [];
    // Loop Data
    dataToExport.forEach((item, index) => {
      const rowData = [item.id, item.name, item.age, item.job,item.path,item.img];
      tableRows.push(rowData);
    });
    // Add multi Image
     // Create an array to hold image promises
     const imagePromises = tableRows.map(async (row) => {
      const imgUrl = row[5];      
    ...
    doc.autoTable({
      ...
      didDrawCell: (data) => {
        ...
        if (column.index === 4) {
          const { img } = images.find(image => image.row[4] === cell.raw) || {};// row[4] show data in there
          
          if (img) {
            const x = cell.x + (cell.width / 2) - 10; // Center the image
            const y = cell.y + (cell.height / 2) - 10; // Center the image
            doc.addImage(img, 'JPEG', x+2 , y+1, 17, 17); // Adjust size as needed
          }
        }
      },
      startY: 30,
      ...
      headStyles:{
        ...
        fontSize: 12,
        halign:'center',
        fillColor:[51,156,255]              
      },
      bodyStyles:{
        textColor:[0,0,0],
        halign:'center',
        lineColor:[51,156,255],
        lineWidth:0.1,
        minCellHeight: 20
      },
      // set data style in body
      columnStyles: { 
        0: { cellWidth:10},
    ...
    const contentArray = Array.from({ length: 5 }
      , (_, i) => ''
    ...
    contentArray.forEach((content, index) => {
      ...
      doc.text(content, 10, 10 + (index % 2) * 10); // Adjust as needed
      // Add a new page after every 5 items
      if ((index + 1) % 5 === 0 && index < contentArray.length - 1) {
        doc.addPage();
      }
    });
    // Finalize page numbers on the last page
    const totalPages = doc.internal.getNumberOfPages();
    for (let i = 1; i <= totalPages; i++) {
      doc.setPage(i); // Set to the current page
      doc.setFontSize('10');
      doc.setTextColor(0, 0, 0);
      doc.text(`Page ${i} of ${totalPages}`, 102, doc.internal.pageSize.height - 10);
    }
    // show print dailog when open PDF file
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <table border="1" cellPadding="10" cellSpacing="0" style={{ width: '100%', marginBottom: '20px', fontFamily: 'Phetsarath OT' }}>
        <thead border="1">
          <tr>
            <th>ID</th>
            <th>Name</th>
